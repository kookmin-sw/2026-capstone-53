...
> **버전**: v1.1.37-MVP
> **최종 수정**: 2026-05-16 (이상진 — §6.1 Route 정합성 2건. (1) ODsay 401/403 auth 에러 시 stale cache fallback 차단 — 운영자 alert 가 다른 응답에 가려져 영구 잠복하던 결함을 즉시 503 격상으로 가시화. (2) `RouteResponse.calculatedAt` non-null invariant — corrupted cache (route_summary_json 있고 route_calculated_at NULL) 가 응답에 silent leak 차단.)
