...
> **버전**: v1.1.39-MVP
> **최종 수정**: 2026-05-16 (황찬우 — PR #54 follow-up. (1) `backend/docs/cross-check-frontend.md` v1.1.37 audit 의 push 도메인 사실 오류 4건 정정 — DELETE path (`/push/subscribe/{subscriptionId}` singular), `PUSH_SUBSCRIBE_CONFLICT` 응답 코드 (503 SERVICE_UNAVAILABLE) 및 원인 (UPSERT race contention), 존재하지 않는 `/push/vapid` endpoint 표기 삭제, RouteController/ScheduleController scheduleId strict validate 적용 여부 (둘 다 미적용). (2) `GeocodeService.callKakao` 에 `catch (BusinessException be) { throw be; }` 명시 가드 추가 — v1.1.37 의 `catch (RuntimeException)` 가 향후 도메인 BusinessException 까지 silent 502 변환할 위험 차단. 회귀 가드 `geocode_KakaoLocalClient_BusinessException_시_원래_코드_propagate` 1건.)
