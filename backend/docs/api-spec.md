...
> **버전**: v1.1.38-MVP
> **최종 수정**: 2026-05-16 (이상진 — fresh 코드리뷰 P2 3건 fix. (1) `§7.1 PushSubscribeRequest.keys.p256dh/auth` base64url `@Pattern` 가드 — RFC 4648 §5 alphabet 미준수 시 발송 단계 좀비 row 생성하던 결함 fail-fast 400 변환. (2) `§4.1 NearestScheduleDto` corrupted coordinate WARN 에 `side=ORIGIN|DESTINATION` 추가 — 4 케이스 단일 메시지로 묻혀 운영 진단 시 DB 재조회 필요하던 결함. (3) `OdsayClient.maskCoord` / `TmapClient.maskCoord` `Locale.ROOT` 명시 — JVM default locale 의존으로 `37,5*` 류 직렬화 silent break 잠재 차단.)
